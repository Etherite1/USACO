...
// template insertion begin



// template insertion end
...
// ll div(ll a, ll b) {return (a * modinv(b, MOD)) % MOD;}
...
vector<vvl> dp(1005, vvl(1005, vl(2, 1e9)));

void solve()
{
    setIO("cowrun");
    int n; cin >> n;
    vl a(n + 4); for(int i = 1; i <= n; i++) cin >> a[i];
    n++;
    sort(a.begin() + 1, a.begin() + n + 1);
    for(int i = 1; i <= n; i++) if(a[i] == 0) dp[i][1][0] = 0;
    for(int len = 1; len < n; len++)
    {
        int ccount = n - len;
        for(int i = 1; i + len <= n + 1; i++)
        {
            dp[i - 1][len + 1][0] = min(dp[i][len][0] + ccount * (a[i] - a[i - 1]), dp[i][len][1] + ccount * (a[i + len - 1] - a[i - 1]));
            dp[i][len + 1][1] = min(dp[i][len][0] + ccount * (a[i + len] - a[i]), dp[i][len][1] + ccount * (a[i + len] - a[i + len - 1]));
        }
    }
