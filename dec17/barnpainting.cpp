...
{
    for(auto adjnode : adj[curr]) if(adjnode != par) dfs(adjnode, curr);
    for(int c = 1; c <= 4; c++)
    {
        if(dp[curr][c] == 0) continue;
        for(auto adjnode : adj[curr])
        {
            if(adjnode != par)
            {
                ll tmp = 0;
                for(int nc = 1; nc <= 4; nc++) if(nc != c) tmp = (tmp + dp[adjnode][nc]) % MOD;
                dp[curr][c] = (dp[curr][c] * tmp) % MOD;
            }
        }
    }
}
...
void solve()
{
    ...
    for(int i = 0; i < k; i++)
    {
        int a, b; cin >> a >> b; 
        col[a - 1] = b;
        for(int j = 1; j <= 3; j++) if(j != b) dp[a - 1][j] = 0;
    }
    dfs(0, -1);
